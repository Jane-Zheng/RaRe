...
def load_collections(file_path: str):
    """
    只保留 text 非空的文档
    topiocqa collection_tsv 用的是 text 列
    """
...
# =========================
# the newest version: Query+CTX:q:q+a:a)+CTX:q:q+a:a)+…+REWRITE:
...
#                          max_concat_length=512, max_query_length=64, per_turn_max_length=128):# old version
#     """
#     TopiOCQA:
#     - 历史按 QA turn 成对处理
#     - 最近 turn 优先（倒序截断）
...
def make_preprocess_function_ppo(tokenizer, max_ctx_length=512, max_suffix_length=64):
    def preprocess_function_ppo(examples): # topiocqa
        ...
        for history_queries, history_answers, query, answer, rewrite_label, posdoc_id in zip(
            examples["history_query"],
            examples["history_answer"],
            examples["query"],
            examples["answer"],
            examples["rewrite_prompt"], # rewrite by gpt or hunman ?
            examples['pos_docs_id']
        ):
            # full_context =[q +''+ a + '|' for q, a in reversed(list(zip(history_queries, history_answers)))]
            full_context = [f"{q} {a}|" for q, a in zip(reversed(history_queries), reversed(history_answers))]
            ...
            input_id = build_input_topiocqa(
                ...
                current_query=query,
                tokenizer=tokenizer,
                max_concat_length=max_ctx_length,
                max_query_length=max_suffix_length,
                per_turn_max_length=128
            )
            ...
            answer_label_text.append(answer) # answer 就是QA的answer！！！
            pos_docs_pids.append(posdoc_id)
            cur_utt_text.append(query)
...
def build_rag_prompt(query: str, docs: List[str]) -> str: # 0330修正
    ...
    system_prompt = """You are a factual and very smart assistant. Understand the provided knowledge and Strictly answer the query based ONLY on the provided knowledge. 
	                       If the knowledge does not contain the answer, say "UNANSWERABLE" only.
	                       Rules:
	                         1. Keep answers precise and short only include the key words or conclusion.
	                         2. Think carefully and Do not answer "UNANSWERABLE" easily.
	                         3. Do not speculate or use your own knowledge.
	                         4. Quote directly from the knowledge when possible.
                         """
...
def combine_rewards(
    ...
    total = np.tanh(total) # 
    # total = np.log(1 + total) # 什么时候解除了注释？？？我记得是注释掉的啊。。。。。这可怎么办
...
def compute_ref_penalty(old_token_logprobs, ref_token_logprobs, mask):
    # diff = torch.abs(old_token_logprobs - ref_token_logprobs) * mask # abs?
    diff = (old_token_logprobs - ref_token_logprobs) * mask
    ...
    while global_step < args.total_steps:
        ...
        for batch in tqdm(train_dataloader, desc=f"ppo-train-epoch-{epoch}"):
            ...
            reward_tensor = torch.tensor(
                [float(r.item() - rr.item()) for r, rr in zip(rewards, ref_rewards)],# 0410 无奈修改。。。希望渺茫
                # [float(x.item()) for x in rewards], # 0412_1
                dtype=torch.float32,
                device=args.policy_device,
            )
            ...
            #         seq_rewards=reward_tensor,   # [B]
            #         token_kl=kl_penalty,           # [B, T]
            #         mask=mask,                   # [B, T]
            #         kl_coef=args.kl_coef,
            #     )
            ...
            # advantages, returns = compute_token_level_advantages_from_returns(
